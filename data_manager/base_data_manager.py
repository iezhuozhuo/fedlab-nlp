...
class BaseDataManager(ABC):
    ...
    def _load_federated_data_local(self, decompose=None, stand_alone=False):
        ...
        # self.logger.info("self.client_index_list = " + str(self.client_index_list))
        ...
        if state:
            ...
            dev_examples, dev_features, dev_dataset = None, None, None
            for i,client_idx in enumerate(self.client_index_list):
                ...
                train_examples[client_idx], train_features[client_idx], train_dataset[client_idx] = \
                    self.preprocessor.transform(**train_data, index_list=train_index_list)

                test_examples[client_idx], test_features[client_idx], test_dataset[client_idx] = \
                    self.preprocessor.transform(**test_data, index_list=test_index_list, evaluate=True)

                if i == 0:
                    dev_list = partition_file[partition_method]["partition_data"][str(client_idx)]["valid"]
                    dev_data = self.read_instance_from_data_file(
                        data_file, dev_list,
                        desc=" train data of client_id=%d " % client_idx)

                    dev_examples, dev_features, dev_dataset = \
                        self.preprocessor.transform(**dev_data, index_list=dev_list, evaluate=True)
        ...
        if stand_alone:
            ...
            test_data_num = len(test_data_global) * self.eval_batch_size * 4
            train_data_global, train_data_num = None, 0
            for key in train_examples:
                train_data_num += len(train_examples[key])
            dev_data_num, dev_data_global = len(dev_examples), None
        else:
