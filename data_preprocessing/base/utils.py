...
def load_glove_embedding(path, source_vocab, dimension, pretrained_dict=None):
    # fedavg 正常 Load embedding
    # source_vocab=None 全部使用 pretrained embedding
    # source_vocab!=None 今初始化 client自己的词表
    vocab = dict()
    weights = []
    pre_cnt, us_cnt = 0, 0
...
def client_random_load_glove_embedding(pretrain_path, source_vocab, dimension):
    logger = registry.get("logger")
    glove_sizes = ["6", "42", "840"]
    clients_vocab = defaultdict(dict)
    clients_weights = defaultdict(list)
    # args = registry.get("args")
    # dimension = 200 if size == "27" else 300
    ...
    for client_idx in client_id_list:
        size = random.sample(glove_sizes, 1)[0]
