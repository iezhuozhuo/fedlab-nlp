...
class TextClassificationTrainer:
    ...
    def train_model(self, device=None):
        ...
        self.logger.info("train_model self.device: " + str(device))
        ...
        for epoch in range(0, self.args.epochs):
            ...
            for batch_idx, batch in enumerate(self.train_dl):
                ...
                self.logger.info("epoch = %d, batch_idx = %d/%d, loss = %s" % (epoch, batch_idx,
                                                                               len(self.train_dl), current_loss))
                if self.wandb:
                    self.wandb.log({"Training Loss": current_loss})
    ...
    def eval_model(self, epoch=0, global_step=0, device=None):
        ...
        self.logger.info("best_accuracy = %f" % self.best_accuracy)

        if self.wandb:
            self.wandb.log({"Evaluation Accuracy (best)": self.best_accuracy})
            self.wandb.log({"Evaluation Accuracy": result["acc"]})
            self.wandb.log({"Evaluation Loss": result["eval_loss"]})

        self.results.update(result)
        self.logger.critical(f"CurAcc: {self.results['acc']}, CurLoss: {self.results['eval_loss']}")
    ...
    def build_optimizer(self, model, iteration_in_total):
        ...
        if self.args.fl_algorithm == "FedOPT" or self.args.fl_algorithm == "":
            self.logger.warning("Using AdamW as Optimizer")
            optimizer = AdamW(model.parameters(), lr=self.args.learning_rate, eps=self.args.adam_epsilon)
        else:
            self.logger.warning("Using SGD as Optimizer")
